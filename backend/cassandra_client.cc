...
int Cassandra::save_message_to_db(const string &user1, const string &user2, const string &sender, const string &message)
{
	...
	cass_statement_bind_int64(statement, 2, ts);
	cass_statement_bind_string(statement, 3, sender.c_str());
	cass_statement_bind_string(statement, 4, message.c_str());
...
{
	cout << "[Cassandra] Getting friends from database for user " << username << endl;
	...
	while (cass_iterator_next(rows))
	{
		...
		cout << "[Cassandra] Found friends for username " << username << ":" << friend_name << endl;
	}
...
int Cassandra::get_user_from_db(const string &username, const string &password)
{
	...
	while (cass_iterator_next(rows))
	{
		...
		{
			cout << "[Cassandra] LOGIN: found user" << endl;
			cass_iterator_free(rows);
			cass_result_free(result);
			cass_future_free(query_future);
...
string Cassandra::get_chat_history_from_db(string username, string _friend, int64_t paging_timestamp)
{
	...
	cout << "[Cassandra] Getting chat history from Cassandra for: " << username << " friend: " << _friend << " timestamp: " << paging_timestamp << endl;
	...
	cout << "[Cassandra] Returning chat history: " << chat_history.str() << endl;
